...
void map_rehash(map_t *map, void **entries, size_t new_cap)
{
    for (size_t i = 0; i < map->cap; i++) {
        ...
        if (map->entries[i] != NULL) {
            if ((err = _object_insert(map, entries, new_cap, map->entries[i])) == -1) {
...
void *map_remove(map_t *map, void *unit)
{
    ...
    for (size_t i = 0; i < map->cap; i++) {
        ...
        if (NULL == map->entries[table_index])
        {
            continue;
        }
        else
        {
            ...
            if (map->compare_cb(unit, map->entries[table_index], NULL) == true) {
                removed_unit = map->entries[table_index];
                map->entries[table_index] = NULL;
                map->count--;
                map_realloc(map);
...
bool map_foreach(map_t *map, bool (*iter_cb)(const void *, const void *), void *udata)
{
    for (size_t i = 0; i < map->cap; i++) {
        if (map->entries[i] != NULL) {
            // printf("in scan: %zu = %p\n", i, &i);
            if (!iter_cb(map->entries[i], udata))
                return false;   
        }
    }
